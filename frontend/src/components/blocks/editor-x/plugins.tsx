...
import {
    MathBlockPickerPlugin,
    MathInlinePickerPlugin,
} from "@/components/editor/plugins/picker/math-picker-plugin";
...
import { InsertMath } from "@/components/editor/plugins/toolbar/block-insert/insert-math";
...
import {
    BLOCK_MATH,
    INLINE_MATH,
} from "@/components/editor/transformers/markdown-math-transformer";
...
/** Configuration for which actions to hide */
...
export function Plugins({ resizable = false, maxLength = 500, autoFocus = false, hideActions = {} }: PluginsConfig) {
        ...
        <div className="relative">
            <ToolbarPlugin>
                {({ blockType }) => (
                    <div
                        className={cn(
                            "vertical-align-middle sticky top-0 z-10 flex items-center overflow-auto border-b",
                            sizeConfig.toolbarPadding,
                            sizeConfig.toolbarGap
                        )}
                        role="toolbar"
            ...
            <div
                className={cn(
                    ...
                    sizeConfig.contentMinHeight,
                    resizable && "max-h-[80vh]"
                )}
            >
                        ...
                        <div className="h-full">
                            <div className="h-full" ref={onRef}>
                                <ContentEditable
                                    ...
                                    className={cn(
                                        "ContentEditable__root relative block h-full overflow-auto focus:outline-none",
                                        sizeConfig.contentPadding,
                                        sizeConfig.fontSize
                                    )}
                                    placeholderClassName={cn(
                                        sizeConfig.placeholderPadding,
                                        sizeConfig.fontSize
                                    )}
                                />
                            </div>
            ...
            <ActionsPlugin>
                <div
                    className={cn(
                        "clear-both flex items-center justify-between overflow-auto border-t",
                        sizeConfig.actionsBarPadding,
                        sizeConfig.toolbarGap
                    )}
                >
                    ...
                    <div className="flex flex-1 justify-end">
                        {!hideActions.speechToText && <SpeechToTextPlugin />}
                        {!hideActions.share && <ShareContentPlugin />}
                        {!hideActions.importExport && <ImportExportPlugin />}
                        {!hideActions.markdownToggle && (
                        ...
                        {!hideActions.editModeToggle && <EditModeTogglePlugin />}
                        {!hideActions.clearEditor && (
                            <>
                                <ClearEditorActionPlugin />
                                <ClearEditorPlugin />
                            </>
                        )}
                        {!hideActions.treeView && <TreeViewPlugin />}
                    </div>
                </div>
            </ActionsPlugin>
        </div>
